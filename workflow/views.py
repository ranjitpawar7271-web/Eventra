...
def _visible_events(user):
    ...
    # Plain participants get a "Participant Calendar" — only events
    # they're actually registered for, not every published event on the
    # platform. Matches the spec's "Display registered/upcoming events",
    # distinct from the organizer's broader operational calendar below.
    return qs.filter(registrations__user=user, registrations__status='confirmed').distinct()
...
def _visible_tasks(user):
    """Event checklist/task deadlines — the "Event Checklist / Milestones"
    half of the organizer calendar (tasks.Task doubles as the event
    checklist; see tasks/models.py's own docstring on that)."""
    from tasks.models import Task
    qs = Task.objects.exclude(due_date__isnull=True).select_related('event')
    if user.is_super_admin or user.is_staff_role:
        return qs
    from django.db.models import Q
    return qs.filter(Q(event__organizer=user) | Q(assigned_to=user))
...
def _collect_agenda(user, range_start, range_end_exclusive):
    ...
    tasks = _visible_tasks(user).filter(due_date__gte=range_start, due_date__lt=range_end_exclusive)
    for task in tasks:
